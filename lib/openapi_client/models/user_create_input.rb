  ...
  class UserCreateInput
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::UserCreateInput` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::UserCreateInput`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'social_accounts')
        self.social_accounts = attributes[:'social_accounts']
      end
    ...
    def hash
      [first_name, last_name, phone_number, timezone, password, level, title, company_name, company_url, verified_at, social_accounts, two_factor_auth, avatar, emails, locked, customdata].hash
    end
