=begin
#Metal API

#This is the API for Equinix Metal Product. Interact with your devices, user account, and projects.

The version of the OpenAPI document: 1.0.0
Contact: support@equinixmetal.com
Generated by: https://openapi-generator.tech
OpenAPI Generator version: 5.1.0-SNAPSHOT

=end

require 'spec_helper'
require 'json'

# Unit tests for OpenapiClient::HardwareReservationsApi
# Automatically generated by openapi-generator (https://openapi-generator.tech)
# Please update as you see appropriate
describe 'HardwareReservationsApi' do
  before do
    # run before each test
    @api_instance = OpenapiClient::HardwareReservationsApi.new
  end

  after do
    # run after each test
  end

  describe 'test an instance of HardwareReservationsApi' do
    it 'should create an instance of HardwareReservationsApi' do
      expect(@api_instance).to be_instance_of(OpenapiClient::HardwareReservationsApi)
    end
  end

  # unit tests for find_hardware_reservation_by_id
  # Retrieve a hardware reservation
  # Returns a single hardware reservation
  # @param id HardwareReservation UUID
  # @param [Hash] opts the optional parameters
  # @option opts [Array<String>] :include Nested attributes to include. Included objects will return their full attributes. Attribute names can be dotted (up to 3 levels) to included deeply nested objects.
  # @option opts [Array<String>] :exclude Nested attributes to exclude. Excluded objects will return only the href attribute. Attribute names can be dotted (up to 3 levels) to exclude deeply nested objects.
  # @return [Device]
  describe 'find_hardware_reservation_by_id test' do
    it 'should work' do
      # assertion here. ref: https://www.relishapp.com/rspec/rspec-expectations/docs/built-in-matchers
    end
  end

  # unit tests for find_project_hardware_reservations
  # Retrieve all hardware reservations for a given project
  # Provides a collection of hardware reservations for a given project.
  # @param id Project UUID
  # @param [Hash] opts the optional parameters
  # @option opts [Array<String>] :include Nested attributes to include. Included objects will return their full attributes. Attribute names can be dotted (up to 3 levels) to included deeply nested objects.
  # @option opts [Array<String>] :exclude Nested attributes to exclude. Excluded objects will return only the href attribute. Attribute names can be dotted (up to 3 levels) to exclude deeply nested objects.
  # @option opts [Integer] :page Page to return
  # @option opts [Integer] :per_page Items returned per page
  # @return [HardwareReservationList]
  describe 'find_project_hardware_reservations test' do
    it 'should work' do
      # assertion here. ref: https://www.relishapp.com/rspec/rspec-expectations/docs/built-in-matchers
    end
  end

  # unit tests for hardware_reservations_id_move_post
  # Move a hardware reservation
  # Move a hardware reservation to another project
  # @param id Hardware Reservation UUID
  # @param project_id Project UUID
  # @param [Hash] opts the optional parameters
  # @return [HardwareReservation]
  describe 'hardware_reservations_id_move_post test' do
    it 'should work' do
      # assertion here. ref: https://www.relishapp.com/rspec/rspec-expectations/docs/built-in-matchers
    end
  end

end
